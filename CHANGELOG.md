...
## 2026-07-22 12:01
- **fix(sanitize): Allow inline SVG in article content** — `svg` was incorrectly listed in `DISALLOWED_TAGS`, causing all AI-generated SVG diagrams to be silently stripped before saving. Added safe SVG child-tag stripping (`script`, `use`, `foreignObject`, etc.) to maintain XSS protection while keeping valid diagrams intact.
- **fix(ai): Validate Fal.ai image download response** — when downloading generated images from Fal.ai's CDN, the code never checked `response.ok` or content-type. This caused error HTML/text to be stored as a JPEG file (21-byte "Internal server error" files visible in GCS). Now throws immediately on non-ok status or non-image content-type, so the catch block skips the broken image instead of persisting it.
