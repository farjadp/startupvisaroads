...
import { addAiKeywords, clearCompletedKeywords } from '@/app/actions/aiActions';
...
export default function AiWriterUI({ queuedKeywords }: any) {
  const [activeTab, setActiveTab] = useState<'AUTO' | 'URL' | 'TEXT'>('AUTO');
    ...
    <div className="bg-white rounded-2xl border border-[#1a1a1a]/5 shadow-sm overflow-hidden">
      <div className="flex flex-col md:flex-row border-b border-[#1a1a1a]/5">
        <button onClick={() => setActiveTab('AUTO')} className={`flex-1 py-4 px-6 font-sans text-sm font-bold uppercase tracking-widest flex items-center justify-center gap-2 transition-colors ${activeTab === 'AUTO' ? 'bg-[#CCFF00] text-[#1a1a1a]' : 'text-[#1a1a1a]/40 hover:bg-[#1a1a1a]/[0.02]'}`}>
          <Calendar className="w-4 h-4" /> Auto-Pilot
        </button>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
            <div>
              ...
              <form action={addAiKeywords}>
                ...
                <button type="submit" className="bg-[#1a1a1a] text-[#F2F0E9] px-6 py-3 font-sans text-sm font-bold uppercase tracking-widest hover:bg-[#CCFF00] hover:text-[#1a1a1a] transition-colors rounded-xl flex items-center gap-2">
        ...
        {(activeTab === 'URL' || activeTab === 'TEXT') && (
          <form onSubmit={handleGenerate}>
            <h3 className="font-serif text-2xl mb-4 text-[#1a1a1a]">
              {activeTab === 'URL' ? 'Rewrite from URL' : 'Rewrite from Raw Text'}
            </h3>
            <p className="font-sans text-[#1a1a1a]/60 text-sm mb-6">
              {activeTab === 'URL' ? 'Paste a link to any article. We will scrape it and rewrite it completely using GPT-4o, generating a brand new Fal.ai cover image.' : 'Paste raw text below. We will expand and optimize it into a full article with SEO structure and images.'}
            </p>
