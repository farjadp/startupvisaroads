  ...
  return (
    <div className="w-full px-4 md:px-8 border-x border-[#1a1a1a]/10 max-w-[1400px] mx-auto bg-[#F2F0E9] font-vazir">
      ...
      <section className="py-16 grid grid-cols-1 md:grid-cols-2 gap-px bg-[#1a1a1a]/20 border border-[#1a1a1a]/20 my-16">
        {c.channels.map((ch) => {
          const ext = /^(https?|mailto):/.test(ch.cta.href);
          ...
          return (
            <div key={ch.n} className="bg-[#F2F0E9] p-8 md:p-12 flex flex-col justify-between min-h-[300px]">
              ...
              <div className="mt-8 flex flex-wrap gap-6">
                {ext ? (
                  <a href={ch.cta.href} {...(ch.cta.href.startsWith('mailto:') ? {} : { target: '_blank', rel: 'noopener noreferrer' })} className={cls}>
                    {ch.cta.href.startsWith('mailto:') ? <Mail className="w-4 h-4" /> : ch.cta.href.includes('wa.me') ? <MessageCircle className="w-4 h-4" /> : <Send className="w-4 h-4" />}
                    <bdi dir="ltr">{ch.cta.label}</bdi>
                  </a>
                ...
                {'secondary' in ch && ch.secondary && (
                  <a href={ch.secondary.href} {...(/^https?:/.test(ch.secondary.href) ? { target: '_blank', rel: 'noopener noreferrer' } : {})} className="text-sm text-[#1a1a1a]/60 hover:text-[#1a1a1a] self-end" dir="ltr">{ch.secondary.label}</a>
                )}
              </div>
            </div>
          );
      ...
      <section id="form" className="py-16 border-t border-[#1a1a1a]/10 grid grid-cols-1 lg:grid-cols-12 gap-12 scroll-mt-28">
